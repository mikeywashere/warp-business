...
using AddMemberRequest = WarpBusiness.Api.Controllers.AddMemberRequest;
...
public class CustomFieldsControllerTests : IClassFixture<WarpTestFactory>
{
        ...
        CreateSameTenantClientsAsync()
    {
        ...
        var tenants = await mineResponse.Content.ReadFromJsonAsync<IEnumerable<WarpBusiness.Shared.Auth.TenantSummaryDto>>();
        ...
        var selectResponse = await setupClient.PostAsJsonAsync("api/auth/select-tenant",
            new WarpBusiness.Shared.Auth.SelectTenantRequest(tenantId));
